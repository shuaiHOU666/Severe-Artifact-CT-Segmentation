...
def main(args,logger):
    ...
    elif args.netType == "unet":
        net = U_Net(1,args.classes)args.netType == "dpt":
...
def goTrain(args,
    ...
    torch.tensor([1.216,0.395,3.673,0.573,14.193,1.798]).reshape(-1,1).to(args.device)
    ...
    # model_Path = '../checkpoint/255-710-20-core_seg/wdice/mla/lora_small_maxiou_train.pth'
    # lora_Path = '../checkpoint/255-710-20-core_seg/wdice/mla/lora_small_maxiou_train_lora.pth'
    # # #run demo directly
    model_Path = '../checkpoint/255-710-20-core_seg/wdice/mla/lora_small_maxiou_train.pth'
    lora_Path = '../checkpoint/255-710-20-core_seg/wdice/mla/lora_small_maxiou_train_lora.pth'
    ...
    for epoch in range(1,epochs+1):
        ...
        with tqdm(total = len(train_set),desc=f'Epoch {epoch}/{epochs}',unit = 'img') as t:
            ...
            plt.subplot(131)
            # origindata = data
            data = np.squeeze(data[:, 0, :, :].detach().cpu().numpy())
            # import pdb;pdb.set_trace()

            plt.imshow(data[0], cmap='gray')
            plt.title(f'Origin slice')

            cmin = 0;
            cmax = 2

            plt.subplot(132)
            predict = np.squeeze(preds.detach().cpu().numpy())
            plt.imshow(predict[0], vmin=cmin, vmax=cmax, cmap='jet')
            plt.title('Predicted Slice')
            plt.subplot(133)
            target = np.squeeze(label.detach().cpu().numpy())
            plt.imshow(target[0], vmin=cmin, vmax=cmax, cmap='jet')
            plt.title('Label Slice ')
                # plt.show()
                # import pdb;
                # pdb.set_trace()
            plt.savefig(('../Tensorboard/' + args.dataset + '/' + f'NA-714-Epoch{epoch}_train_comparision.jpg'),
...
def get_args():
    ...
    parser.add_argument('--loss', '-loss',  type=str, default='wdice')
    ...
    parser.add_argument('--netType', '-net',  type=str, default='mla', help='pup,mla,linear,unet')
    parser.add_argument('--dataset', '-d', type=str, default='core_seg')
    parser.add_argument('--device', '-dn', type=str, default='cuda')
